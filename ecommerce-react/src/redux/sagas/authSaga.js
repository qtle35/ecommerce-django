...
function* authSaga({ type, payload }) {
  switch (type) {
    case SIGNIN:
      try {
        yield initRequest();
        const userCredential = yield call(userService.signIn, payload.email, payload.password);
        const userId = userCredential.data.user_id;
        const snapshot = yield call(userService.getUser, userId);
        yield put({ type: ON_AUTHSTATE_SUCCESS, payload: { id: userId} });
      } catch (e) {
        yield handleError(e);
      }
      break;
    ...
    case ON_AUTHSTATE_SUCCESS: {
      console.log(payload)
      const snapshot = yield call(userService.getUser, payload.id);
      ...
      if (snapshot.data) { // if user exists in database
        ...
        yield put(signInSuccess({
          id: user.id,
          role: user.is_manager ? 'ADMIN' : 'USER',
          provider: 'password'
        }));
      ...
      yield put(setAuthenticating(false));
      // yield put({ type: SET_AUTH_PERSISTENCE });
      break;
    }
    ...
    case SET_AUTH_PERSISTENCE: {
      ...
      const { basket, profile, auth, checkout } = state;
      try {
        const persistedState = { basket, profile, auth, checkout };
        console.log(persistedState)
        localStorage.setItem('hi', JSON.stringify(persistedState));
