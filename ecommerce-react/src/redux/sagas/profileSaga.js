  ...
  switch (type) {
    ...
    case UPDATE_PROFILE: {
      try {
        ...
        const { avatar, banner, ...updatesWithoutAvatarAndBanner } = payload.updates;
        const updates = { ...updatesWithoutAvatarAndBanner };
        if (avatarFile || bannerFile) {
          const bannerURL = bannerFile ? yield call(userService.storeImage, state.auth.id, bannerFile, 'banner') : payload.updates.banner;
          const avatarURL = avatarFile ? yield call(userService.storeImage, state.auth.id, avatarFile, 'avatar') : payload.updates.avatar;

          const response = yield call(userService.updateProfile, state.auth.id, updates);
